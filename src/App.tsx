...
export default function App() {
  ...
  const [editingSessionInfo, setEditingSessionInfo] = useState<Partial<Session> | null>(null);
  const [isNewSessionFlow, setIsNewSessionFlow] = useState(false);

  const handleConfirmSessionInfo = async (data: Partial<Session>) => {
    if (isNewSessionFlow) {
      ...
      <main className="max-w-2xl mx-auto px-6 pb-32">
        {view === 'list' ? (
                    ...
                    {sortSessions(
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-white/40 mt-1 truncate">{session.subtitle || new Date(session.date).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}</p>
                        </div>
          ...
          <SessionDetail
            ...
            onEditSessionInfo={() => setEditingSessionInfo(selectedSession)}
          />
        )}
      </main>

      {/* Session Info Modal (Create/Edit) */}
      {editingSessionInfo && (
        <SessionInfoModal
          initialData={editingSessionInfo}
          isNew={isNewSessionFlow}
          onConfirm={handleConfirmSessionInfo}
          onCancel={() => {
            setEditingSessionInfo(null);
            setIsNewSessionFlow(false);
          }}
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div>
        ...
        <p
          className="text-xl font-bold text-white cursor-pointer hover:text-white/80 transition-colors flex flex-col group w-max"
          onClick={() => {
            if (session.isDemo) {
